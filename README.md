...
## Привязка к прошивке ЭБУ

В корне лежат `J5TRS251_VOLVO_ORIG.bin` (прошивка ЭБУ) и `firmware_cal.ini`
(калибровки, привязанные именно к ней). Заголовок `include/firmware_cal.h` делается
ОТДЕЛЬНОЙ командой (не при каждой сборке):

```
python tools/fw_cal/gen_fw_cal.py
```

Идентификация прошивки и число цилиндров берутся из самого `.bin`,
калибровки ДАД и объём — из ini. Без сгенерированного заголовка сборка идёт
на значениях по умолчанию (`include/firmware_cal_default.h`).

В ini записана контрольная сумма прошивки. Поменяешь прошивку в блоке —
сборка это заметит, громко предупредит и плата скажет об этом в Serial
строками `[FW]`. Подробности и порядок действий при смене прошивки — в
`docs/KLINE.md`, раздел «ПРИВЯЗКА К ПРОШИВКЕ ЭБУ».
