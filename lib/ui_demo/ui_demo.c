...
void ui_demo_create(void) {
  ...
  // Стартовая вкладка — ONLINE (индекс 1), а не DEBUG. DEBUG стоит первым
  // исторически, с этапа отладки K-Line, но в машине нужна приборка.
  // Без анимации: это не переход по нажатию, а начальное состояние, и
  // прокручивать его на глазах у пользователя незачем.
  //
  // ВАЖНО: звать только ПОСЛЕ screen_*_create. Таймеры экранов выходят
  // сразу, если их вкладка не видна (lv_obj_is_visible), а видимость
  // считается по факту раскладки — до создания содержимого переключать
  // нечего.
