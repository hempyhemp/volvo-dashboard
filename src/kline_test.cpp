...
// 2026-09-19: снижено до 75мс по просьбе пользователя. Ровно столько же
// держит ИОН (InjOnl.ini ReqTimeout=45/ReadTimeout=45, но по сниферу
// фактический отклик ЭБУ ~48мс) — запас над откликом остаётся, а «мёртвое»
// ожидание после промаха сократилось вдвое. Если пойдут обрезки кадра
// (n<64) — вернуть 90..120.
...
// 2026-09-19: снижено до 45мс по просьбе пользователя. Это возможно только
// потому, что чередование с чтением давления снято (KLINE_MAP_FROM_FRAME):
// раньше на один кадр уходило два слота, теперь один. Зазор P3 до следующего
// запроса при отклике ~48мс становится маленьким — если полезут промахи
// опроса или переподключения, поднять до 60..100. Тайминги правятся и на
// лету кнопками ±5 на вкладке DEBUG.
#define KLINE_POLL_INTERVAL_MS 45
    ...
    if (kline_parse_read_data(g_buf, g_buf_n, &kd)) {
      ...
      if (now_ms - last_tune >= 1000) {
        ...
        Serial.printf(
            ...
            "воздух=%ld.%ld кг/ч баро=%ld.%02ld буст=%ld.%02ld бар\n",
            ...
            (long)((kd.map_kpa_x100 - kd.baro_kpa_x100) / 100),
            (long)(labs(kd.map_kpa_x100 - kd.baro_kpa_x100) % 100));
        // КАЛИБРОВКА ДАВЛЕНИЯ. F80C (АЦП ДАД Hi прямо из кадра, resp[38])
        // по логу 2026-09-19 идёт параллельно F9A0 со смещением +9..11
        // единиц, то есть давление МОЖНО считать прямо из кадра и совсем
        // отказаться от чередования с SID 0x23, удвоив частоту опроса.
        // Печатаем обе версии кПа: если расхождение на ходу держится в
        // пределах пары кПа — переходим на кадр.
        // "дельта16" проверяет гипотезу F9A1 == round(F9A0/16) (то есть
        // что F9A1 — грубая копия, а НЕ старший байт). Не 0 — вернуться к теме.
        {
          // Обе версии по калибровке от ИОН: рабочая — из кадра (F80C),
          // справочная — из F9A0 (тот же закон, F9A0 идёт на ~10 единиц ниже).
          int32_t p_f80c = kline_map_from_adc(g_buf[38]);
          int32_t p_f9a0 = kline_map_from_adc((uint8_t)(g_map_b0 + 10));
          Serial.printf(
              "[CAL] F80C=%u->%ld.%02ld кПа | F9A0=%u->%ld.%02ld кПа | "
              "F9A1=%u (ожид %u, дельта16=%d)\n",
              g_buf[38], (long)(p_f80c / 100), (long)(labs(p_f80c) % 100),
              g_map_b0, (long)(p_f9a0 / 100), (long)(p_f9a0 % 100), g_map_b1,
              (unsigned)((g_map_b0 + 8) / 16),
              (int)g_map_b1 - (int)((g_map_b0 + 8) / 16));
        }
